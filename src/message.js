...
/*
	* Create By Maxis
	* Follow https://github.com/maxisdev
	* Whatsapp : https://whatsapp.com/channel/0029VaWOkNm7DAWtkvkJBK43
*/

async function GroupUpdate(maxis, update, store, groupCache) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(maxis, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db.groups && global.db.groups[id] && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await maxis.profilePictureUrl(n, 'image');
				} catch {
...
async function LoadDataBase(maxis, m) {
	try {
...
async function MessagesUpsert(maxis, message, store, groupCache) {
	try {
		let botNumber = await maxis.decodeJid(maxis.user.id);
		const msg = message.messages[0];
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) {
			store.groupMetadata ??= await maxis.groupFetchAllParticipating().catch(e => ({}))
		}
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!msg.key.fromMe && !msg.message && message.type === 'notify') return
		const m = await Serialize(maxis, msg, store, groupCache)
		require('../maxis')(maxis, m, message, store, groupCache);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			await maxis.appendResponseMessage(m, JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id);
		}
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await maxis.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) maxis.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await maxis.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(maxis, store) {
	maxis.serializeM = (m) => MessagesUpsert(maxis, m, store)
	...
	maxis.getName = (jid, withoutContact  = false) => {
		const id = maxis.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || maxis.groupMetadata(id) || {};
	...
	maxis.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await maxis.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await maxis.getName(i + '@s.whatsapp.net')}\nFN:${await maxis.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await maxis.getName(i + '@s.whatsapp.net')}\nFN:${await maxis.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/maxis_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		maxis.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	maxis.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await maxis.query({
			tag: 'iq',
			attrs: {
	...
	maxis.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return maxis.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted.expiration || 0 })
	}
	
	maxis.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return maxis.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return maxis.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return maxis.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('image')) {
				return maxis.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('video')) {
				return maxis.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('audio')) {
				return maxis.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			}
		}
	...
	maxis.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await maxis.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	maxis.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await maxis.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	maxis.sendTextMentions = async (jid, text, quoted, options = {}) => maxis.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	maxis.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return maxis.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	maxis.downloadMediaMessage = async (message) => {
	...
	maxis.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await maxis.downloadMediaMessage(message);
	...
	maxis.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: maxis.user.id, quoted: m.quoted });
		apb.key = m.key
		apb.key.fromMe = areJidsSameUser(m.sender, maxis.user.id);
		if (m.isGroup) apb.participant = m.sender;
		maxis.ev.emit('messages.upsert', {
			...m,
	...
	maxis.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await maxis.getFile(path, true);
		...
		let anu = await maxis.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	maxis.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await maxis.relayMessage(msg.key.remoteJid, msg.message, {
	...
	maxis.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	maxis.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: maxis.waUploadToServer });
			const anu = await maxis.query({
				tag: 'message',
			...
			const _query = await maxis.query({
				tag: 'iq',
				attrs: {
					to: 's.whatsapp.net',
					type: 'get',
	...
	maxis.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: maxis.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await maxis.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (maxis.user && maxis.user.id) {
		const botNumber = maxis.decodeJid(maxis.user.id);
		if (global.db.set && global.db.set[botNumber]) {
			maxis.public = global.db.set[botNumber].public
		} else maxis.public = true
	} else maxis.public = true
...
async function Serialize(maxis, m, store, groupCache) {
	const botNumber = maxis.decodeJid(maxis.user.id)
	if (!m) return m
	...
	if (m.key) {
		...
		m.sender = maxis.decodeJid(m.fromMe && maxis.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await maxis.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = groupCache.get(m.chat))
			if (!metadata) {
				metadata = await maxis.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = maxis.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === maxis.decodeJid(maxis.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, maxis)
				return await Serialize(maxis, q, store, groupCache)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(maxis.decodeJid(m.msg?.contextInfo?.participant), maxis.decodeJid(maxis?.user?.id)),
			...
			m.quoted.download = () => maxis.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				maxis.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => maxis.downloadMediaMessage(m)
	
	m.copy = () => Serialize(maxis, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return maxis.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
		} else if (typeof content === 'string') {
			return maxis.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
		}
	}
